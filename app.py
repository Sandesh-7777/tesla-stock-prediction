...
@st.cache_resource
def load_scaler():
    return joblib.load("tesla_scaler.pkl")

@st.cache_resource
def load_prediction_model(horizon):
    model_files = {
        "1 Day Ahead": "tesla_best_model_1d.keras",
        "5 Days Ahead": "tesla_best_model_5d.keras",
        "10 Days Ahead": "tesla_best_model_10d.keras"
    }
    return load_model(model_files[horizon])
...
with st.spinner("Loading model..."):
    selected_model = load_prediction_model(horizon)
...
next_day_model = load_prediction_model("1 Day Ahead")

next_pred_scaled = next_day_model.predict(
    last_60_reshaped,
    verbose=0
)
